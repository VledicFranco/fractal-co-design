...
description: Scan a project with fca-index, query for code context, and understand discovery vs production mode.
---
...
- `better-sqlite3` native module. This installs automatically with `npm install @fractal-co-design/fca-index` if your platform has Node.js native build tools. On Windows, install Visual Studio Build Tools first.
...
{
  "dependencies": {
    "@fractal-co-design/fca-index": "workspace:*"
  }
}
```
...
```typescript
import { createFcaIndex } from '@fractal-co-design/fca-index';
import { InMemoryIndexStore } from '@fractal-co-design/fca-index/index-store';
import { NodeFileSystem } from '@fractal-co-design/fca-index/cli';
import { DefaultManifestReader } from '@fractal-co-design/fca-index/cli';
import { VoyageEmbeddingClient } from '@fractal-co-design/fca-index/index-store';
...
For testing consumers of the external ports (not internals), use `RecordingContextQueryPort` from `@fractal-co-design/fca-index/testkit` — see `src/testkit/README.md`.
