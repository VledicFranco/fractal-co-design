> **Canonical FCD PRD.** Migrated from `method/docs/prds/054-fca-index-mcp-tools.md` on 2026-04-25. Defines the MCP-side context-query / coverage-check tools that wrap `@fractal-co-design/fca-index` (formerly `@methodts/fca-index`) ports. Original specification preserved verbatim — references to `@methodts/*` reflect the upstream implementation history.
