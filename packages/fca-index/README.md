...
`@fractal-co-design/fca-index` replaces that with a semantic index. After a one-time scan, a typed query returns a ranked list of component descriptors — paths, part locations, and excerpts — without reading any source files. The agent reads only the files it selects from the results.
...
|-----------|-------|
| Layer | L3 — library |
| Dependencies | `@fractal-co-design/fca-index` has zero dependencies on `@methodts/mcp` or `@methodts/bridge` |
